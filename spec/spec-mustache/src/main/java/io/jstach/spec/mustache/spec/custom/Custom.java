...
public class Custom {
	...
	@JStache(path = InterpolationSpecTemplate.DOTTED_NAMES___BASIC_INTERPOLATION_FILE)
	public record DottedNamesBasicInterpolation(Person person) {
	}

	@JStache(path = InterpolationSpecTemplate.DOTTED_NAMES___AMPERSAND_INTERPOLATION_FILE)
	public record DottedNamesAmpersandInterpolation(Person person) {
	}

	@JStache(path = InterpolationSpecTemplate.DOTTED_NAMES___TRIPLE_MUSTACHE_INTERPOLATION_FILE)
	public record DottedNamesTripleMustacheInterpolation(Person person) {
	}

	@JStache(path = SectionsSpecTemplate.CONTEXT_FILE)
	public record Context(Person context) {
	}

	@JStache(path = InvertedSpecTemplate.DOTTED_NAMES___TRUTHY_FILE)
	...
	// @GenerateRenderableAdapter(template =
	// InterpolationSpecTemplate.DOTTED_NAMES___BROKEN_CHAIN_RESOLUTION_FILE)
