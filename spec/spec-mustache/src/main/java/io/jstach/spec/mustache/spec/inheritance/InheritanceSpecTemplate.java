...
public enum InheritanceSpecTemplate implements SpecListing {
	...
	DEFAULT(Default.class, "inheritance", "Default", "Default content should be rendered if the block isn't overridden",
	...
	TRIPLE_MUSTACHE(TripleMustache.class, "inheritance", "Triple Mustache",
			"Default content renders triple mustache variables", "{\"bar\":\"<baz>\"}",
			"{{$foo}}default {{{bar}}} content{{/foo}}\n", "default <baz> content\n", Map.of()) {
		public String render(Map<String, Object> o) {
	...
	SECTIONS(Sections.class, "inheritance", "Sections", "Default content renders sections",
			"{\"bar\":{\"baz\":\"qux\"}}", "{{$foo}}default {{#bar}}{{baz}}{{/bar}} content{{/foo}}\n",
			"default qux content\n", Map.of()) {
		public String render(Map<String, Object> o) {
			var m = new Sections();
			m.putAll(o);
	...
	NEGATIVE_SECTIONS(null, "inheritance", "Negative Sections", "Default content renders negative sections",
			"{\"baz\":\"three\"}", "{{$foo}}default {{^bar}}{{baz}}{{/bar}} content{{/foo}}\n",
			"default three content\n", Map.of()) {
		public String render(Map<String, Object> o) {
			return "DISABLED TEST";
		}
	},
	MUSTACHE_INJECTION(MustacheInjection.class, "inheritance", "Mustache Injection",
			"Mustache injection in default content", "{\"bar\":{\"baz\":\"{{qux}}\"}}",
	...
	OVERRIDDEN_CONTENT(Overriddencontent.class, "inheritance", "Overridden content", "Overridden content", "{}",
			"{{<super}}{{$title}}sub template title{{/title}}{{/super}}", "...sub template title...", Map.of(
	...
	DATA_DOES_NOT_OVERRIDE_BLOCK(Datadoesnotoverrideblock.class, "inheritance", "Data does not override block",
			"Context does not override argument passed into parent", "{\"var\":\"var in data\"}",
			"{{<include}}{{$var}}var in template{{/var}}{{/include}}", "var in template", Map.of(
	...
	DATA_DOES_NOT_OVERRIDE_BLOCK_DEFAULT(Datadoesnotoverrideblockdefault.class, "inheritance",
			...
			"{\"var\":\"var in data\"}", "{{<include}}{{/include}}", "var in include", Map.of(
	...
	OVERRIDDEN_PARENT(Overriddenparent.class, "inheritance", "Overridden parent", "Overridden parent", "{}",
			"test {{<parent}}{{$stuff}}override{{/stuff}}{{/parent}}", "test override", Map.of(
	...
	OVERRIDE_PARENT_WITH_NEWLINES(Overrideparentwithnewlines.class, "inheritance", "Override parent with newlines",
			"Override parent with newlines", "{}", "{{<parent}}{{$ballmer}}\npeaked\n\n:(\n{{/ballmer}}{{/parent}}",
			"peaked\n\n:(\n", Map.of(
	...
	INHERIT_INDENTATION(Inheritindentation.class, "inheritance", "Inherit indentation",
			...
			"{{<parent}}{{$nineties}}hammer time{{/nineties}}{{/parent}}", "stop:\n  hammer time\n", Map.of(
	...
	ONLY_ONE_OVERRIDE(Onlyoneoverride.class, "inheritance", "Only one override",
			...
			"{{<parent}}{{$stuff2}}override two{{/stuff2}}{{/parent}}", "new default one, override two", Map.of(
	...
	PARENT_TEMPLATE(Parenttemplate.class, "inheritance", "Parent template",
			...
			"{{>parent}}|{{<parent}}{{/parent}}", "default content|default content", Map.of(
	...
	RECURSION(null, "inheritance", "Recursion", "Recursion in inherited templates", "{}",
			"{{<parent}}{{$foo}}override{{/foo}}{{/parent}}", "override override override don't recurse", Map.of(

					"parent", "{{$foo}}default content{{/foo}} {{$bar}}{{<parent2}}{{/parent2}}{{/bar}}", "parent2",
					"{{$foo}}parent2 default content{{/foo}} {{<parent}}{{$bar}}don't recurse{{/bar}}{{/parent}}")) {
		public String render(Map<String, Object> o) {
			return "DISABLED TEST";
		}
	},
	MULTI_LEVEL_INHERITANCE(Multilevelinheritance.class, "inheritance", "Multi-level inheritance",
			"Top-level substitutions take precedence in multi-level inheritance", "{}",
			"{{<parent}}{{$a}}c{{/a}}{{/parent}}", "c", Map.of(

					"parent", "{{<older}}{{$a}}p{{/a}}{{/older}}", "older",
					"{{<grandParent}}{{$a}}o{{/a}}{{/grandParent}}", "grandParent", "{{$a}}g{{/a}}")) {
		public String render(Map<String, Object> o) {
	...
	MULTI_LEVEL_INHERITANCE__NO_SUB_CHILD(Multilevelinheritancenosubchild.class, "inheritance",
			...
			"Top-level substitutions take precedence in multi-level inheritance", "{}", "{{<parent}}{{/parent}}", "p",
			Map.of(
	...
	TEXT_INSIDE_PARENT(Textinsideparent.class, "inheritance", "Text inside parent",
			...
			"{{<parent}} asdfasd {{$foo}}hmm{{/foo}} asdfasdfasdf {{/parent}}", "hmm", Map.of(
	...
	TEXT_INSIDE_PARENT1(Textinsideparent1.class, "inheritance", "Text inside parent1",
			"Allows text inside a parent tag, but ignores it", "{}", "{{<parent}} asdfasd asdfasdfasdf {{/parent}}",
			"default content", Map.of(
	...
	BLOCK_SCOPE(Blockscope.class, "inheritance", "Block scope",
			...
			"{{<parent}}{{$block}}I say {{fruit}}.{{/block}}{{/parent}}", "I say bananas.", Map.of(
	...
	private InheritanceSpecTemplate(Class<?> modelClass, String group, String title, String description, String json,
