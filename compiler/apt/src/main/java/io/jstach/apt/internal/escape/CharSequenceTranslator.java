...
abstract class CharSequenceTranslator {
	 ...
	 * into another set of code points. The number of code points consumed must be
	 * returned, and the only IOExceptions thrown must be from interacting with the Writer
	 * so that the top level API may reliably ignore StringWriter IOExceptions.
